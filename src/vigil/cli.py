...
def report(file: str, name: str, output: str):
    ...
    path = Path(file)
    project_name = name or path.stem
    ...
    # Generate markdown
    lines = [
        f"# {project_name} — Dependency Health Report",
        f"",
        f"> Generated by [vigil-risk](https://pypi.org/project/vigil-risk/) "
        f"on {datetime.now(timezone.utc).strftime('%Y-%m-%d')}.",
        f"",
        f"| Package | Risk | Score | Top Concern |",
        f"|---------|------|-------|-------------|",
    ]
    ...
    for p in sorted_profiles:
        level = p.risk_level.value.upper()
        top = p.signals[0].detail[:60] if p.signals else "—"
        lines.append(f"| {p.package} | {level} | {p.risk_score:.2f} | {top} |")
    ...
    lines.extend([
        f"",
        f"**Summary:** {len(result.profiles)} packages scanned. "
